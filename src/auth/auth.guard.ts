...
import { ROLE_PERMISSION, USER_BLOCKED_ERROR_DESCRIPTION, USER_BLOCKED_ERROR_MESSAGE } from "./auth.constant";
...
export class AuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (rawToken) {
            try {
                ...
                const tokenObj = await this.repository.userToken.findToken(rawToken)
                ...
                throw new UnauthorizedException(INVALID_TOKEN_ERROR_MESSAGE, UNAUTHORIZED_ERROR_DESCRIPTION)
            } catch (error) {
                let errorMessage = ""
                let errorDescription = UNAUTHORIZED_ERROR_DESCRIPTION
                if (error.name === 'TokenExpiredError')
                    errorMessage = "Token kedaluwarsa"
                else if ("response" in error && !!error.response && "message" in error.response)
                    errorMessage = error.response.message
                else
                    errorMessage = INVALID_TOKEN_ERROR_MESSAGE

                if ("response" in error && !!error.response && "description" in error.response)
    ...
    private async checkUserBlockStatus(userId: string) {
        const blacklistedToken = await this.repository.userToken.findUserBlacklistedToken(userId)
        if (blacklistedToken)
            await this.repository.userToken.updateUserActiveTokensToBlacklisted(userId)
